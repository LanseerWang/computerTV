...
from config import DEFAULT_CONFIGS, reg, APP_NAME, APP_PATH
...
def get_active_network_info():
    ...
    for interface_name, addresses in interfaces.items():
        ...
        # 检查接口是否处于活动状态
        if psutil.net_if_stats()[interface_name].isup and 'Loopback' not in interface_name:
            info = {'interface': interface_name, 'mac': '', 'ipv4':'', 'ipv6':''}
            for address in addresses:
                if str(address.family) == 'AddressFamily.AF_INET':
                    info['ipv4'] = str(address.address)
                elif str(address.family) == 'AddressFamily.AF_INET6':
                    info['ipv6'] = str(address.address)
                elif str(address.family) == 'AddressFamily.AF_LINK':
                    info['mac'] = str(address.address)
            available_interfaces.append(info)
...
def start_app():
    ...
    s = socketserver.ThreadingUDPServer(('0.0.0.0', 10088), MyServer)
    s.serve_forever()
...
def install():
    ...
    print(mktasks_cmd.format(install_path=APP_PATH))
    os.system(mktasks_cmd.format(install_path=APP_PATH))
    # 配置环境变量
...
def showQRInfo():
    interface_infos = get_active_network_info()
    print(interface_infos)
    qr = qrcode.QRCode(
        version=1,
